...
7)JSON: If you need to store data in key-value structure you can use JSON data type
		...
		--TRUNCATE command removes all data from table.
--What is the difference between DELETE and TRUNCATE command?
--Delete command deletes just the records, it does not destroy the table structure
--1)DELETE FROM deletes the data by filtering the data with WHERE clause, but in TRUNCATE command you cannot use WHERE clause
--2)IF you use DELETE FROM, you can reach out to that data after removing it but when you use TRUNCATE all the data will be deleted and
--you cannot reach them again.
    */
}
